...
namespace soa.qa.services.Questions
{
    public class CreateQuestionProcessor : ICreateQuestionProcessor
    {
        ...
        public CreateQuestionProcessor(IUnitOfWork uOf, IAutoMapper autoMapper,
            IQuestionRepository questionRepository, IPersonRepository personRepository,
        ...
        public Task<QuestionUiModel> CreateQuestionAsync(QuestionForCreationUiModel newQuestionUiModel)
        {
            ...
            try
            {
                ...
                foreach (var tagId in newQuestionUiModel.TagIds)
                {
                    var tagToBeInjected = _tagRepository.FindBy(tagId);

                    if (tagToBeInjected == null)
                        throw new TagDoesNotExistException(tagId);

                    var tagQuestionToBeAdded = new TagQuestion()
                    {
                        Tag = tagToBeInjected
                    };

                    questionToBeCreated.InjectWithTagQuestion(tagQuestionToBeAdded);
                }
